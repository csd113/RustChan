...
mod tests {
    ...
    fn detect_empty_is_error() {
        assert!(super::mime::detect_mime_type(b"").is_err());
    }

    #[test]
    fn detect_jpeg() {
        assert_eq!(
            super::mime::detect_mime_type(b"\xff\xd8\xff\xe0rest").expect("jpeg"),
            "image/jpeg"
        );
    }
    ...
    fn detect_png() {
        assert_eq!(
            super::mime::detect_mime_type(b"\x89PNG\r\n\x1a\nrest").expect("png"),
            "image/png"
        );
    }
    ...
    fn detect_gif() {
        assert_eq!(
            super::mime::detect_mime_type(b"GIF89arest").expect("gif"),
            "image/gif"
        );
    }
    ...
    fn detect_wav() {
        assert_eq!(
            super::mime::detect_mime_type(b"RIFF\x00\x00\x00\x00WAVErest").expect("wav"),
            "audio/wav"
        );
    }
    ...
    fn detect_mp3_raw_sync() {
        assert_eq!(
            super::mime::detect_mime_type(b"\xff\xfbrest").expect("mp3"),
            "audio/mpeg"
        );
    }
    ...
    fn detect_aac() {
        assert_eq!(
            super::mime::detect_mime_type(b"\xff\xf1rest").expect("aac"),
            "audio/aac"
        );
    }

...
(function() {{
  ...
  // Based on the reference implementation of FIPS 180-4.
  function sha256Fallback(str) {{
    ...
    for (var i = 0; i < padded.length; i += 64) {{
      ...
      for (var j = 16; j < 64; j++) {{
        var s0 = r(7,w[j-15]) ^ r(18,w[j-15]) ^ (w[j-15] >>> 3);
        var s1 = r(17,w[j-2]) ^ r(19,w[j-2]) ^ (w[j-2] >>> 10);
  ...
  function sha256(str) {{
    // Prefer the native Web Crypto API (requires secure context / HTTPS).
    // Fall back to the pure-JS implementation when crypto.subtle is unavailable
    // (e.g. plain HTTP), so PoW solving still works in development/local setups.

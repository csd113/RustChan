...
pub fn nsfw_disclaimer_page(
    ...
    let body = format!(
        r#"<div class="compress-modal" style="display:flex">
  <div class="compress-modal-box nsfw-disclaimer-box">
    ...
    <div class="compress-modal-info">
      <p style="margin-bottom:0.8rem">To access this section, you understand and agree to the following:</p>
      <ol class="nsfw-disclaimer-list">
        ...
        <li>This website is presented to you AS IS, with no warranty, express or implied. By clicking &quot;I Agree,&quot; you agree not to hold 4chan responsible for any damages from your use of the website, and you understand that the content posted is not owned or generated by 4chan, but rather by 4chan&apos;s users.</li>
        <li>As a condition of using this website, you agree to comply with the &quot;Rules&quot; of 4chan, which are also linked on the home page. Please read the Rules carefully, because they are important.</li>
      </ol>
    </div>
    <div class="compress-modal-actions">
      <form method="POST" action="/nsfw/accept" style="display:inline-flex">
        <input type="hidden" name="_csrf" value="{csrf}">
        <input type="hidden" name="return_to" value="{return_to}">
        <button type="submit" class="compress-do-btn">I Agree</button>
      </form>
      <a class="compress-cancel-btn btn" href="/">Cancel</a>
    </div>
  </div>
</div>"#,
        csrf = escape_html(csrf_token),
    ...
    base_layout(
        &format!("Disclaimer — /{}/", board.short_name),
        Some(&board.short_name),
        &body,
        csrf_token,
        boards,
